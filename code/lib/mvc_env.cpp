...
//      return -(double)(getMaxConnectedNodesNum()/graph->num_nodes)*(graph->nodes_weight[a]/graph->total_nodes_weight);
    double raw_reward = -(double)getMaxConnectedNodesNum()/(double)graph->num_nodes-(double)graph->nodes_weight[a]/(double)graph->total_nodes_weight;
    return raw_reward;
}
