...
// 性价比 reinsert 策略: 返回"组件数"(该节点加入时会连接到的不同连通片数量)。
// 在 reInsert_inner 中会再除以节点成本 -> 优先级 = 组件数 / cost。
class decreaseComponentCountCost : public decreaseComponentStrategy
{
public:

	decreaseComponentCountCost() :decreaseComponentStrategy(){}
