...
std::vector<int> Utils::reInsert(std::shared_ptr<Graph> graph,std::vector<int> solution,const std::vector<int> allVex,int decreaseStrategyID,int reinsertEachStep){
    ...
    switch(decreaseStrategyID){
        ...
        case 4:{
            // 性价比策略: 组件数 / cost (cost 在 reInsert_inner 中除)
            decreaseStrategy = std::shared_ptr<decreaseComponentStrategy>(new decreaseComponentCountCost());
            break;
        }
        default:
        {
...
std::vector<int> Utils::reInsert_inner(const std::vector<int> &beforeOutput, std::shared_ptr<Graph> &graph, const std::vector<int> &allVex, std::shared_ptr<decreaseComponentStrategy> &decreaseStrategy,int reinsertEachStep)
{
    ...
    Disjoint_Set disjoint_Set =  Disjoint_Set(graph);
...
//            int left_node_num = graph->num_nodes - leftOutput.size()+1;
            double node_weight = graph->nodes_weight[eachNode];
            if(node_weight==0)
            {
                node_weight = 0.000000000000001;
            }
//            node_weight = 1;
            batchList.push_back(make_pair((double)decreaseValue/node_weight, eachNode));
        }
...
//        printf("Node:%d\n",(solution.size()-1-i));
        if (i == 0)
         ...
         {
            solution_weights = graph->nodes_weight[solution[i-1]];
         }
        graphutil->recoverAddNode(backupCompletedAdjListGraph, backupAllVex, current_adj_list, Node, disjoint_Set);
        int n_left = graph->num_nodes - i;
        double n_gcc = (double)disjoint_Set.maxRankCount/n_left;

        double node_cost = (double)solution_weights/(double)graph->total_nodes_weight;
        cost_list.push_back(node_cost);

        totalMaxNum += n_gcc*node_cost;
//        totalMaxNum += (double)disjoint_Set.maxRankCount * solution_weights;

        MaxWccSzList.push_back(n_gcc);
    }
...
//    totalMaxNum = totalMaxNum - disjoint_Set.maxRankCount;
    ...
    std::reverse(cost_list.begin(), cost_list.end());

    //modify score start
    double totalScore = 0;
    double prev_gcc = 1.5;
    std::vector<double> modify_MaxWccSzList;
    for (int i = 0; i < (int)cost_list.size(); i++)
    {
        double current_cost = cost_list[i];
        double current_ngcc = MaxWccSzList[i];
//        current_ngcc = (current_ngcc<prev_gcc)?current_ngcc:prev_gcc;
        modify_MaxWccSzList.push_back(current_ngcc);
        totalScore += current_cost * current_ngcc;
        prev_gcc = current_ngcc;
    }
    MaxWccSzList = modify_MaxWccSzList;
    totalMaxNum = totalScore;
    //modify score end
